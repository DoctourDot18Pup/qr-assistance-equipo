...
import { importGroupStudents, type ImportRow, type ImportResult } from "./actions";

type Step = "upload" | "preview" | "results";

const TEMPLATE = `nombre,matricula\nAna Martínez,21031001\nCarlos Ruiz,21031002`;
...
function parseCSV(text: string): ImportRow[] {
  ...
  return lines
    ...
    .map((line) => {
      ...
      const enrollmentNumber = v[idx("matricula")] ?? "";
      return {
        name:             v[idx("nombre")] ?? "",
        email:            enrollmentNumber ? `${enrollmentNumber}@itcelaya.edu.mx` : "",
        enrollmentNumber,
      };
    })
    .filter((r) => r.name || r.enrollmentNumber);
}
...
export function ImportGroupStudentsClient({ groupId }: Props) {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      ...
      if (parsed.length === 0) {
        setError("No se encontraron filas válidas. Verifica que el archivo tenga las columnas: nombre, correo, matricula.");
        return;
      }
      setError("");
      setRows(parsed);
  ...
  async function handleImport() {
    ...
    setError("");
    try {
      const res = await importGroupStudents(groupId, rows);
      setResults(res);
      setStep("results");
    } catch (err: unknown) {
  ...
  const added   = results.filter((r) => r.status === "created" || r.status === "enrolled").length;
        ...
        <div className="bg-white border border-[#D8CFB8] rounded-[6px] p-10">
          <div className="max-w-sm mx-auto text-center">
            ...
            <div className="text-xs text-[#6B6457] mb-5">
              Columnas requeridas: <span className="font-mono">nombre, matricula</span>
              <br />El correo se genera automáticamente como <span className="font-mono">matrícula@itcelaya.edu.mx</span>
              <br />Si el alumno no existe, se crea con contraseña <span className="font-semibold">Bienvenido123</span>
            </div>
        ...
        <div className="bg-white border border-[#D8CFB8] rounded-[6px]">
          <div className="px-[18px] py-4 border-b border-[#D8CFB8] flex items-center justify-between">
            <span className="text-sm font-semibold text-[#0A0A0A]">
              {rows.length} alumno{rows.length !== 1 ? "s" : ""} listos para inscribir
            </span>
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-[#D8CFB8] text-[11px] font-semibold uppercase tracking-wide text-[#6B6457]">
                  ...
                  <th className="text-left px-4 py-3">Correo (generado)</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className={i % 2 === 1 ? "bg-[#F5F1EA]" : ""}>
                    ...
                    <td className="px-4 py-2.5 tabular text-[#6B6457]">
                      {r.enrollmentNumber || <span className="text-[#7A1A1A]">falta</span>}
                    </td>
                    <td className="px-4 py-2.5 text-[#6B6457]">
                      {r.email || <span className="text-[#7A1A1A]">falta matrícula</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="px-[18px] py-4 border-t border-[#D8CFB8] flex justify-end gap-2">
            ...
            >
              {pending ? "Inscribiendo…" : `Inscribir ${rows.length} alumnos`}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white border border-[#D8CFB8] rounded-[6px] px-5 py-4">
              <div className="text-[11px] font-semibold uppercase tracking-wide text-[#6B6457]">Añadidos</div>
              <div className="text-2xl font-semibold text-[#1B3A2D] tabular mt-1">{added}</div>
            </div>
            <div className="bg-white border border-[#D8CFB8] rounded-[6px] px-5 py-4">
              <div className="text-[11px] font-semibold uppercase tracking-wide text-[#6B6457]">Ya inscritos</div>
              <div className="text-2xl font-semibold text-[#B8965A] tabular mt-1">{already}</div>
            </div>
            <div className="bg-white border border-[#D8CFB8] rounded-[6px] px-5 py-4">
          ...
          <div className="bg-white border border-[#D8CFB8] rounded-[6px]">
            <div className="px-[18px] py-4 border-b border-[#D8CFB8]">
              <span className="text-sm font-semibold text-[#0A0A0A]">Detalle por alumno</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-[13px]">
                <thead>
                  ...
                  {results.map((r, i) => (
                    <tr key={i} className={i % 2 === 1 ? "bg-[#F5F1EA]" : ""}>
                      <td className="px-[18px] py-2.5">
                        ...
                        {r.status === "enrolled" && (
                          <span className="text-[11px] font-semibold text-[#1B3A2D]">✓ Inscrito</span>
                        )}
                        {r.status === "already" && (
                          <span className="text-[11px] font-semibold text-[#B8965A]">— Ya inscrito</span>
                        )}
                        {r.status === "error" && (
